...
import helpers.mongo.pmpermitdb as Flaming
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await Flaming.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Flaming.set_permit_message(Flaming.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await Flaming.set_permit_message(f"`{arg}`")
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Flaming.set_block_message(Flaming.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Flaming.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Flaming.get_pm_settings()
    await Flaming.allow_user(chat_id)
